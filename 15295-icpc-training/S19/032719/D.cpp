...
int main(){
	...
	sort(v,v+n,cmp);
	st[stn++]=STA(-1, 0);
	st[stn++]=STA(0, (ll)v[0].x*v[0].y-v[0].val);
	dp[1] = (ll)v[0].x*v[0].y-v[0].val;
	dp[0] = 0;
	ll curmax = dp[1];
	int sth = 1;
	for(int i=1;i<n;i++){
//		printf("%d ", stn);
		while(sth<stn && check(sth, v[i].y)>0) sth++;
//		printf("%ld ", k);
		ll k = calcdist(sth-1, v[i].y) + v[i].x*v[i].y - v[i].val;
		dp[i+1] = k;
		if (k > curmax) curmax = k;
		ll u1,u2,v1,v2;
		update(u1, u2, v1, v2);
		while(stn>sth && (ld)(dp[i+1]-v1)/(v[i].x-u1)>=(ld)(v2-v1)/(u2-u1)){
			stn--;
			update(u1, u2, v1, v2);
		}
		st[stn++] = STA(i, dp[i+1]);
